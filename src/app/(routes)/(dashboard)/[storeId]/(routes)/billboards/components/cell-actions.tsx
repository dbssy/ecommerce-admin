...
export function CellActions({ data }: ICellActionsProps) {
  ...
  async function onConfirm() {
    try {
      ...
      toast.success('O anúncio foi deletado com sucesso!');
      router.refresh();
    } catch (error) {
      toast.error('Certifique-se de remover todas as categorias primeiro.');
    } finally {
      setIsOpen(false);
      setIsLoading(false);
    }
  ...
  function onCopy(id: string) {
    ...
    toast.success(
      'O ID do anúncio foi copiado com sucesso para a área de transferência!',
    );
  }
